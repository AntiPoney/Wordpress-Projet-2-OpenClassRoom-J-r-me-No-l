 ...
 */
class Catchadaptive_Json_Ld_Schema {
	private $position = 0;
	...
	function catchadaptive_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'catch-adaptive' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'catch-adaptive' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'catch-adaptive' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'catch-adaptive' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'catch-adaptive' ); // text for an author page
	...
	function json_ld_breadcrumbs() {
		...
		if ( isset( $options['breadcumb_option'] ) && $options['breadcumb_option'] ) {
			$show_on_home = ( isset( $options['breadcumb_onhomepage'] ) && $options['breadcumb_onhomepage'] ) ? 1 : 0;

			$breadcrumb = $this->catchadaptive_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
...
new Catchadaptive_Json_Ld_Schema();
